...
- **i18n**: English/Indonesian via `next-intl`, in "without i18n routing" mode — locale is stored in a `NEXT_LOCALE` cookie (set by the `setUserLocale` Server Action in `src/i18n/locale.ts`), not a `/en`/`/id` URL segment, since this is a single anchor-scroll page rather than a multi-route site. `src/i18n/request.ts` wires the locale into `next-intl/server`'s `getRequestConfig`; `src/app/layout.tsx` reads it via `getLocale()` for `<html lang>` and wraps the tree in `NextIntlClientProvider`.
  - **Structural data vs. copy convention**: `src/lib/site-config.ts` holds only structural, non-translatable data (URLs, images, tags, dates, tech/brand names). Any list item with translatable copy (projects, experience, nav links) carries a stable `id`. All translatable copy lives in `messages/en.json` / `messages/id.json`, namespaced by section and keyed by that same `id` (e.g. `projects.items.bookease.title`). Components merge the two via `useTranslations()` (client) / `getTranslations()` (async server) — never hardcode copy directly in JSX. Keep `messages/en.json` and `messages/id.json` key-for-key in sync; `src/types/next-intl.d.ts` augments next-intl's `AppConfig` from `messages/en.json` so message keys are type-checked.
  - The language switch (`src/components/language-toggle.tsx`, using the shadcn `Switch`) sits next to `ThemeToggle` in both the navbar and mobile drawer, but doesn't need `ThemeToggle`'s hydration-guard pattern — locale is resolved server-side, so there's no client/server mismatch to guard against.
  - Extending this to future sections (e.g. Endorsements, Technical Writing/Journal): add structural entries with a stable `id` in `site-config.ts`, then add matching keys to **both** locale files under a new namespace.
